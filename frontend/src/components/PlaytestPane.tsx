        ...
        {j ? (
          <>
            <div className="flex flex-wrap gap-1.5 text-[10px]">
              ...
              <span className="px-1.5 py-0.5 rounded bg-indigo-950/50 text-indigo-300">{t('playtest.badgePacing')} {j.interactivity_pacing_score}/5</span>
              <span className="px-1.5 py-0.5 rounded bg-indigo-950/50 text-indigo-300">{t('playtest.badgeAgency')} {j.player_agency_score}/5</span>
              {j.dead_end_risk !== 'none' && (
                <span className="px-1.5 py-0.5 rounded bg-amber-950/50 text-amber-300">{t('playtest.deadEndLabel')} {j.dead_end_risk}</span>
              )}
            </div>
          ...
          <p className="text-[11px] text-red-400">{t('playtest.judgeFailed')}{frame.judge_error}</p>
        )}
      </div>
    </div>
  )
...
export default function PlaytestPane({ jobId }: { jobId: string }) {
  ...
  const DIMENSION_LABELS: Record<string, string> = {
    ui_coherence: t('playtest.dimUiCoherence'),
    interactivity_pacing: t('playtest.dimPacing'),
    player_agency: t('playtest.dimPlayerAgency'),
    dead_end_risk_pct: t('playtest.dimDeadEndRisk'),
    coverage: t('playtest.dimCoverage'),
    branch_reachability: t('playtest.dimBranchReachability'),
  }
  ...
  if (!loaded) {
    return <div className="text-xs text-gray-500 p-4">{t('playtest.loading')}</div>
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        >
          {loading ? t('playtest.running') : report ? t('playtest.rerun') : t('playtest.run')}
        </button>
      </div>
      ...
      {report && (
        <>
          ...
          <div className="text-[10px] text-gray-600">
            {report.visited_scenes}/{report.total_scenes} {t('playtest.scenesVisited')} ·
            {' '}{report.reachable_branches}/{report.total_declared_branches} {t('playtest.branchesReachable')} ·
            {' '}{report.frames_judged} {t('playtest.framesJudged')} ({report.frames_skipped} {t('playtest.skipped')}) ·
            {' '}{t('playtest.judgeLabel')} {report.judge_model}
          </div>
          {report.unreachable_scene_ids.length > 0 && (
            <div className="text-[10px] text-amber-400">
              {t('playtest.unreachableScenes')}{report.unreachable_scene_ids.join(', ')}
            </div>
          )}
