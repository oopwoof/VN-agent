...
- **50-scene 长篇**：**编排层已零成本实测**（2026-08-20，`smoke_longvn.py --mock --scenes 50 --concurrent 5`：5 chapters、state_timeline 50 条、DAG waves、writer 峰值并发 5、10 条 callback 冲突 Tier-1 解决，全部断言通过）。但 mock 只验证结构与编排。**12 场的真实长篇数字现在有了**（2026-09-19，text-only、并发 3）：$1.65、墙钟 25.7 分钟、cache_read_ratio 0.20、Reviewer 首轮 PASS avg 4.2、key 轮换 0 次。50-scene 仍是外推——但外推基数已从 6-scene 换成真实的 12-scene 数据（约 $6.9、约 1.8 小时）。**仍未验证**：50 场跨度的上下文退化、图片成本（两次都是 text-only）、Vision Judge。可写“12-scene 真实运行实测 $1.65，据此外推 50-scene 低于 $15”，不能写“已实现 50-scene 成本 $15”。
  - **第一次真实运行把自己的钱赚回来了。** 它花了 $4.29，暴露出三个 mock 永远看不见的缺陷：主题语言从未传到 Writer（中文主题产出 100% 英文剧本——而 mock 按 CJK 分流的 fixture 让 mock 路径看起来一直双语正常）；Writer 把每场 66-100% 的输出预算花在了一个写完即被 `strip_thinking()` 丢弃的 `<thinking>` 块上，导致 12 场里 10 场在 JSON 中途被截断、6 场塌到 5 行下限；Reviewer 随后对这些稀薄场景判 FAIL，系统跑满三轮修订、对着同一个上限重写全部场景——约 $1.10 和 22 分钟，且不可能成功。管线上游其实已经付费跑过独立的 `thinking_fanout` 阶段产出场景计划并注入同一个 prompt，**等于为同一份推理付了两次费，而被牺牲的是对白**。修复后同一条命令花费 **$1.65（−62%）**、耗时 **25.7 分钟（−46%）**、无一场落在下限、首轮评审 4.2 通过。**省下来的不是质量的代价——质量反而上升了，因为预算终于花到了对白上。** 截断现在是一等信号（`output_capped_scenes`），会写进 `run_metrics.json` 并短路修订循环，形状与既有的图类 FAIL 绕过一致。
