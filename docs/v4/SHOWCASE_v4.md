...
| §3A 步骤 5（场景胶片条逐格填充） | ⚠️ 未看到 | 全程停在 `场景 0/5`，完成后直接跳到播放器。**根因未查明**（原先归因于后端阻塞，那个判断已被上面的自我订正推翻）。**别把「边生成边流过来」当卖点讲，除非你自己先复现过** |
| §3B 步骤 2–7（设定确认 → 故事板 → 卡片详情 → 从卡片播放 → Chat Ops 铅笔） | ❌ 未走查 | 扩展中途掉线 |
| `?shell=v1` 逃生口 | ❌ 未复验 | 同上。代码仍在（`useShellVariant.ts`） |
