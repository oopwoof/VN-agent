...
**耗时**：⚠️ **实测 199 秒**（3 场景，mock，2026-08-12）。此前本行写「约 40–90 秒」是低估。
**原因**：每个场景都会重建一次 sentence-transformers 的 lore 检索索引（后端日志里能看到 `BertModel LOAD REPORT` 反复出现），这是同步 CPU 工作跑在 async 事件循环里——**期间整个后端被阻塞**，连 `/status` 都会挂住。
**对演示的影响**：别按 90 秒设计口播节奏，按 3 分钟准备；或者在演示前先跑一次预热，让模型进缓存。这段等待正好用来讲 §3A 步骤 3 的那段话。
...
| §3A 步骤 5（场景胶片条逐格填充） | ⚠️ 未看到 | 全程停在 `场景 0/5`，完成后直接跳到播放器。可能是后端阻塞导致中间态没推上来（见下）。**别把「边生成边流过来」当卖点讲，除非你自己先复现过** |
...
**本次新发现的两个问题**（都已在上面标注）：
1. **后端会被自己阻塞**：每场景重建一次 sentence-transformers 索引，同步 CPU 工作跑在 async 事件循环里，期间 `/status` 等所有请求一起挂住。演示时表现为「看起来卡住了」。
2. **浏览器截图偶发超时**（`Page.captureScreenshot timed out`），大概率是同一个根因——渲染线程等后端响应。重试一次通常就好。
