...
**耗时**（3 场景 mock，2026-08-12 实测，四组对照，全部 `calls: 0`）：

| 形态 | 开浏览器录制 | 服务 | 实测 |
|---|---|---|---|
| 纯文本（`text_only`） | 否 | 热 | **62 秒** |
| Autopilot | 否 | 热 | **90 秒** |
| Autopilot | 否 | 冷启 | **95 秒** |
| Autopilot | **是（边跑边截图）** | 冷启 | **196 秒** |

**结论**：按 **60–95 秒**准备口播节奏。原来那句「约 40–90 秒」基本是对的。
**但演示时如果你在录屏/投屏，预留到 3 分钟**——观测本身会让它慢一倍。
**冷启只比热启慢 5 秒**，所以不必特意预热模型。
...
> ✅ **`workbench-player.jpg` 已于 2026-08-12 重拍**（job `21277806`，主题「樱花树下的转学生」→ 场景「初次相遇」）：主题错位已消除，可以放心放大讲。
> ⚠️ **`workbench-pipeline-theatre.jpg` 仍是旧图**（2026-08-11，主题「黄昏的旧书店」），仍带主题错位，左侧历史栏还能看到 `mock gate check` 这类调试 job。**要么别放大，要么按 §3A 再跑一次重拍。**

### 已验证 / 未验证（2026-08-12 浏览器实机走查）

| 项 | 状态 | 证据 |
|---|---|---|
| PowerShell 启动命令（§1 全部） | ✅ | 后端 200 / 前端 200 |
| 启动 banner 显示 mock 状态 | ✅ | `[vn-agent] VN_AGENT_MOCK='1' -> mock floor ON` |
| v2 工作台是默认外壳 | ✅ | `localStorage` 为空、URL 无参数，直接进 v2 |
| §1 空态三件事 | ✅ | 历史加载 / 「输入一个主题开始生成」/ 底部「就绪」 |
| §3A 步骤 1–2（输入主题 → 一键生成） | ✅ | job `21277806` |
| §3A 步骤 3（流水线剧场节点依次点亮） | ✅ | 导演·结构审校·状态编排·分场推理 变绿，**交叉引用高亮为当前节点** |
| §3A 步骤 4（素材生成「已跳过」） | ✅ | 灰显 + 「已跳过」标签，**不是一直转圈** |
| §3A 步骤 6（零点击进全幅播放器） | ✅ | 生成完成后自动进入，全程未再点击 |
| **零花费** | ✅ | 录制过程中实时查询：`{"calls":0,"estimated_cost_usd":0.0}` |
| 主题 / 标题 / 场景自洽 | ✅ | 樱花树下的转学生 → 《樱花树下的约定》→ 初次相遇。**主题错位已消除** |
| 素材面板空态诚实 | ✅ | 背景(2)/角色(6)/BGM(2)，占位素材明确标「占位图」 |
| **§3B 步骤 8（EN 切换整段历史重译）** | ✅ **最强的一条** | 全部界面文案与历史消息重译；**用户输入的「樱花树下的转学生」与 LLM 生成的「樱花树下的约定」「初次相遇」保持中文不动** |
| §3A 步骤 5（场景胶片条逐格填充） | ✅ 已定位根因（**非 bug**） | 见下方「胶片条根因」。8-12 那次停在 `场景 0/5` 是 **Autopilot 的预期行为**：首场到达即切播放器，胶片条随 PipelineStage 一起卸载。8-11 的非 Autopilot 走查里它确实从 `0/5` 填到 `3/5`（`progress.md:273`）。**可以讲流式，但要讲对：Fast Mode 关闭时看得见填充，Autopilot 下看不见** |
| §3B 步骤 2–7（设定确认 → 故事板 → 卡片详情 → 从卡片播放 → Chat Ops 铅笔） | ⚠️ 8-11 通过，8-12 未复验 | `progress.md:274-285` 记录五个按钮齐全、卡片 3 可从该场开播；8-12 扩展中途掉线未能复验 |
| `?shell=v1` 逃生口 | ⚠️ 8-11 通过，8-12 未复验 | `progress.md:294-297`（根仍是 `bg-gray-950`）+ `progress.md:371-372`（清 localStorage 后再验）。代码仍在（`useShellVariant.ts`） |
| **录屏 GIF** | ❌ **第二次失败** | 导出报告成功（9 帧 / 1109KB），但文件**从未落盘**——Downloads 与整个用户目录下都搜不到。**不再重试，用截图兜底** |

### 胶片条根因（2026-08-24 补，读代码得出，未再开浏览器复现）

`场景 0/5` 不动不是后端没流，是前端把承载它的组件换掉了：

1. `frontend/src/store.ts:252-262`：SSE 收到**第一个**场景时，若 `config.autopilot` 为真，立刻 `set({ vnPreview: true })`——这是 P5 「零点击进播放器」的设计。
2. `frontend/src/shell/WorkbenchShell.tsx:18`：`resolveForm` 里 `if (vnPreview) return 'player'` 优先于一切 `step`。
3. 于是 PipelineStage（胶片条所在）在第 1 场就被卸载，第 2..N 场的填充没有组件可以显示。

所以 8-12 看到的「0/5 然后直接跳播放器」与 8-11 看到的「0/5 → 3/5」并不矛盾——**差别是 Autopilot 开没开**（`progress.md:268` 记录 8-11 那次 Fast Mode 关闭）。

演示时的说法：想展示胶片条填充就关掉 Autopilot 走 §3B；走 §3A 的 Autopilot 路径时，卖点是「零点击直接进播放器」而不是「看着它一格格填」。

**仍待做**：8-12 掉线后 §3B 步骤 2–7 与 `?shell=v1` 一直没在浏览器里复验过（8-11 的记录还在，但那已是两周前的代码）；`docs/v4/assets/workbench-pipeline-theatre.jpg` 仍是 8-11 那张主题错位的旧图。两件事都需要有人开着 Chrome 才能做。

### ⚠️ 自我订正记录（2026-08-12，`d0530fc` 之后）

`d0530fc` 的 commit message 与本文件里，我写过两条**错误结论**，现按证据订正。留档而不是悄悄改掉，因为这个项目的文档一直以「主动记录偏差」为原则。

| 我写过的 | 实际 | 证据 |
|---|---|---|
| 「**每个场景**都会重建一次 sentence-transformers 索引」 | **不是每场景**。lore 索引按设计每 run 建一次（`agents/writer.py:169`，注释明写 per-run）；语料索引默认根本不跑（`config.py:142` `corpus_path` 为空）。那次运行日志里 `BertModel LOAD REPORT` 只出现 **2 次**，不是 3 次（3 场景） | `grep -c` 后端日志 = 2 |
| 「实测 199 秒，此前写『40–90 秒』是**低估**」 | **原文没错，是我的测量被污染了**。196 秒那次是我一边跑一边用浏览器截图（CDP 截图本身还在 30 秒超时）。同形态 headless 重跑是 **95 秒**，纯文本 **62 秒** | 上表四组对照 |

**教训**：我用一个会扰动被测对象的仪器去测它。**带浏览器录制的耗时不能当作产品耗时写进文档。**

顺带否掉的一个假设：**模型加载不是瓶颈**——冷启 95 秒 vs 热启 90 秒，只差 5 秒。（`eval/embedder.py:65` 每次实例化确实都重新加载模型、无缓存，这条仍然成立且值得修，但它不是耗时主因，别当性能故事讲。）

**仍然成立的一个观察**：浏览器截图偶发 `Page.captureScreenshot timed out`，且生成期间 `/status` 会挂住——同步 CPU 工作跑在 async 事件循环上这件事本身是真的，只是量级远小于我原来的判断。重试一次通常就好。
...
- **测试**：**971 passed / 1 skipped / 0 failed**（2026-08-13 单进程全量实跑，共 972 collected）
