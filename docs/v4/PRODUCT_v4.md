...
| **P3** ④Chat Ops M1 | ✅ M1 已提交（2026-08-24） | `chat_ops/{intent_router,orchestrator}.py` + `chat_ops/executors/{add_character,edit_asset}.py` + `POST /api/projects/{id}/chat/{preview,execute}` + `ChatPanel.tsx` 意图确认卡片。**4 个意图现在全部可执行**：local_regen 复用 `agents/local_regen.py`；**add_character** 由 LLM 合成 CharacterProfile→CharacterDesigner 填视觉档案→原子写 characters.json 与剧本 cast（**不改写既有场景**——进 cast 与上台是两回事，回复里说清是哪一件；重名拒绝而非覆盖；非 ASCII id 转写，因为它要当 Ren'Py 变量名）；**edit_asset** 先查开源素材库（命中即零调用换图）再回退 SceneArtist 重生成，立绘编辑明确拒绝并说明理由（单表情重画会与其余表情脱节）。mock 模式下图片生成按设计不可用，执行器写档案并如实说明，而不是把边界报成失败。**L2 置信度阈值已落地**（`chat_intent_confidence_threshold=0.6`）：低置信度的改动型意图变成澄清提问而非确认卡片——L1 只在创作者认真读卡片时才起作用，一个 0.3 置信度的卡片长得和 0.95 的一模一样。L3 top-K / L4 误分类回流飞轮仍是 roadmap |
