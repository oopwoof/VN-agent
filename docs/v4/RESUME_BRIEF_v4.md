...
- **50-scene long-form**: the **orchestration layer is now measured at zero cost** (2026-08-20, `smoke_longvn.py --mock --scenes 50 --concurrent 5`: 5 chapters, 50 state_timeline rows, DAG waves, peak writer concurrency 5, 10 callback conflicts Tier-1-resolved, all assertions green). Mock validates structure and orchestration only — **context degradation, output quality, real cost, and rate-limit behavior remain unverified**. Full 50-scene wall / cost claims are still **projections from 6-scene baseline**: say "50-scene orchestration mock-validated, on track for a real run under $15", not "achieved 50-scene at $15".
