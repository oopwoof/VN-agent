...
**M1 (2026-08-24)**: all 4 intents now execute. `add_character` synthesizes a profile, runs it through CharacterDesigner, and writes characters.json + the script's cast atomically — it deliberately does NOT rewrite existing scenes, because being in the cast and being on stage are different things and the reply says which one happened. `edit_asset` tries the open-source library first (a hit is a zero-call, licence-clean swap) before regenerating. L2 confidence threshold shipped (`chat_intent_confidence_threshold=0.6`): a low-confidence mutating intent becomes a clarifying question instead of a confirm card, because L1 only protects a creator who reads the card and a 0.3-confidence card looks exactly like a 0.95 one. L3 top-K options / L4 feedback-into-P1-flywheel remain roadmap. Browser click-through still not done.
...
- **971 passed / 1 skipped / 0 failed, 972 collected** (2026-08-13, full suite in a single process on `main`). The one skip is `test_real_api.py`, which skips by design without a key. History: 939 (2026-07-29) → 947 (2026-08-10) → 959 → 972.
  > ⚠️ **Which number is which**: 939/947/959 were `--collect-only` *collection* counts, not pass counts; earlier docs rendering "947 passed" were wrong. **Say "~970 tests, 971 passing and 0 failing on the last full run".**
  > **Two problems previously recorded here are now fixed, and both are worth telling as stories**:
  > ① The suite used to die partway in one process with a torch Windows access violation. Root cause: `EmbeddingIndex` rebuilt a SentenceTransformer on every instantiation, crashing inside transformers' `_materialize_copy`. Sharing one per model name fixed it (commit `9ac84ae`).
  > ② `TestWarningsDedup` sat on the "known flaky" list for weeks and **was not flaky — it was calling Anthropic for real**. `ainvoke_with_pending_debug` re-imports `ainvoke_llm` inside its own body, bypassing the test's patch of `structure_reviewer.ainvoke_llm`, so the model answered differently each run (4 findings vs 5). Closed with a two-layer conftest floor (commit `f1dd956`); `test_agents` dropped from 945s to 556s as a side effect.
...
- **50-scene long-form**: the **orchestration layer is now measured at zero cost** (2026-08-20, `smoke_longvn.py --mock --scenes 50 --concurrent 5`: 5 chapters, 50 state_timeline rows, DAG waves, peak writer concurrency 5, 10 callback conflicts Tier-1-resolved, all assertions green). Mock validates structure and orchestration only. **Real long-form numbers now exist at 12 scenes** (2026-09-19, text-only, concurrency 3): $1.65, 25.7 min wall, cache_read_ratio 0.20, Reviewer PASS at avg 4.2 on the first round, zero key rotations. 50-scene is still a projection — but now projected from real 12-scene data (~$6.9, ~1.8h) rather than a 6-scene baseline. **Still unverified**: context degradation across 50 scenes, image cost (both runs were text-only), Vision Judge. Say "12-scene real run measured at $1.65; 50-scene projected under $15 from that", not "achieved 50-scene at $15".
  - **The first real run paid for itself.** It cost $4.29 and exposed three defects mock could never have shown: the theme's language never reached the Writer (a Chinese theme produced a 100% English script — mock's CJK-keyed fixtures made the mock path look correctly bilingual); the Writer spent 66-100% of its per-scene output budget on a `<thinking>` block that `strip_thinking()` discards, so ten of twelve scenes were cut off mid-JSON and six collapsed to the five-line floor; and the Reviewer then FAILed those thin scenes, burning all three revision rounds rewriting every scene against the identical cap — ~$1.10 and 22 minutes that could not have succeeded. The pipeline had already paid a separate `thinking_fanout` stage to produce the plan and injected it into the same prompt, so it bought the same reasoning twice and the dialogue is what got truncated. After the fix, the same command cost **$1.65 (-62%)**, ran in **25.7 min (-46%)**, left no scene at the floor, and passed review on the first round at 4.2. **The saving was not a quality trade — quality went up because the budget finally reached the dialogue.** A truncated response is now a first-class signal (`output_capped_scenes`) that reaches `run_metrics.json` and short-circuits the revision loop, the way graph-class FAILs already did.
  - **The measurement itself was broken until 2026-08-24** — worth telling as its own story rather than hiding. The harness guarded its cost and cache fields behind `hasattr` checks for method names TokenTracker never had, so `run_metrics.json` shipped without either field and the $15 ceiling assertion read a missing key as 0.0. It could never have fired. The health gate had the same shape of problem from the other direction: its wall-clock baseline assumed 18s/scene while the only real run on record measured 61s/scene, so it would have flagged healthy runs RED and, with `--abort-on-degradation`, killed the expensive tier over ordinary throughput. Both fixed, plus a mid-flight `--max-budget-usd` cutoff (post-run assertions cannot stop a run that goes wrong at scene 5 of 50 and keeps paying) and a crash path that still writes the run record. **The lesson to tell: a validation harness needs its own validation — "the assertion passed" and "the assertion ran" are different claims.**
