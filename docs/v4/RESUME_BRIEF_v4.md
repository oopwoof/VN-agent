...
| 3-layer memory for long-form (Character Bible + recursive Haiku summary + sliding window) | Sonnet context isn't infinite; naive full-history injection wastes cache | Character Bible → `cache_control=ephemeral` (first 1.25×, 5min reuse 0.1×). Summarizer gated ≥15 scenes. Sliding window default N=2 | `src/vn_agent/agents/summarizer.py`, `state_orchestrator.py`, Sprint 11-1/11-2/11-3 |
...
- **M0 baseline (6-scene, real API, text-only)**: 38.1 min, $2.04 (`docs/v3/SHOWCASE_v3.md` §6, 2026-04-26)
