...
**首次真实 API 长篇运行（2026-09-19，12 场，$4.29）**：`smoke_longvn.py --scenes 12 --characters 3 --text-only --concurrent 3 --theme 樱花树下的转学生 --max-budget-usd 6`。这是本项目第一次拿到**真实**的成本与缓存数字——此前所有此类字段都是死代码（见上一节）。

| 指标 | 实测 | 对照 |
|---|---|---|
| 总成本 | **$4.29** | 基准估 $3.16、含修订估 $4.11 → **成本模型误差 4%** |
| cache_read_ratio | **0.246** | 首次实测，此前无任何真实值 |
| 墙钟 | 47.8 分钟（239 秒/场） | 健康基线 75 秒/场 → **health RED**，`--abort-on-degradation` 触发 |
| key 轮换 | 0 次 | 单 key + 并发 3 未撞 429，说明 50 场用并发 3 是安全的 |
| 修订轮 | writer ×3 / reviewer ×3，**全部 FAIL** | 跑满 `max_revision_rounds=3`，终稿 avg 3.2 < 阈值 3.5 |

**三个只有真实运行才能暴露的缺陷**：
1. **主题语言未传到 Writer**（已修 `9423158`）：中文主题产出 100% 英文剧本。语言提示原本检测 `script.description`，而 Director 用英文写 description，于是提示从未触发。mock 模式恰恰掩盖了它——`mock_llm._dispatch` 按 prompt 里的 CJK 分流并返回中文 fixture，看起来一直正常。
2. **`writer_max_tokens_per_scene=5000` 静默劣化质量**：12 场里 **10 场**的 Writer 响应在 JSON 中途被截断 → 数组解析失败 → 逐对象兜底只捞回部分行，或直接占位 + 续写补到 5 行下限。结果 **6/12 场恰好停在 5 行下限**，且是连续的最后四场——整个第三幕报废。
3. **修订循环在修一个它修不了的问题**：Reviewer 对那些被截断的稀薄场景判 FAIL，系统于是跑满 3 轮全场重写（多花约 $1.1、22 分钟），但每一轮都会撞上同一个 token 上限。**没有任何信号把「这次响应撞了 max_tokens」告诉评审与修订决策**——成本上限本是安全措施，却成了质量故障的根源，而系统把账算在了 Writer 头上。

**对 50 场档的影响**：按 $0.357/场外推，50 场约 **$17.9**，高于 $15 目标；墙钟按 239 秒/场、并发 3 外推约 **3.3 小时**。这两个数字都建立在「10/12 场被截断 + 跑满 3 轮修订」之上，先修 token 上限与修订触发条件再跑，才有参考价值。运行记录：`demo_output/real_20260919/tier_12/`（run_metrics / trace / run_meta / 逐场 debug 原文）。

**修复后同条件重跑（2026-09-19，12 场，$1.65）**：同主题、同参数，只改了「计划块不再重复推导」与「截断成为一等信号」两处。

| 指标 | 首跑 | 重跑 | 变化 |
|---|---|---|---|
| 成本 | $4.29 | **$1.65** | **-62%** |
| 墙钟 | 47.8 分钟 | **25.7 分钟** | **-46%** |
| health | RED | **green** | — |
| Writer 响应被截断 | 10/12 | **3/12** | — |
| 塌到 5 行下限的场景 | 6/12 | **0/12**（最少 9 行） | — |
| Reviewer | 三轮全 FAIL，终稿 avg 3.2 | **首轮 PASS，avg 4.2** | voice 4 / subtext 5 / arc 4 / pacing 4 |
| 修订轮 | 3 轮（跑满） | **0 轮** | — |
| 对白语言 | 100% 英文 | **86% 中文**（其余为标点与拉丁字符） | — |

省钱的来源不是降低质量，而是**不再为同一份推理付两次费**：thinking_fanout 已经产出场景计划，Writer 却被系统提示要求再自己推导一遍，而那部分输出写完即被 `strip_thinking()` 丢弃。质量反而从 FAIL 升到首轮 PASS——因为预算终于花在了对白上。

**新的一等信号生效**：3 个仍被截断的场景以 `Writer[output-cap]: 3 scene(s) truncated at writer_max_tokens_per_scene=5000: [...]` 出现在 `run_metrics.json` 的 warnings 里，指名道姓给出该调的旋钮。本次因首轮 PASS 未触发跳过修订的分支。

**这次重跑又暴露一个**：对白是中文，但标题出来是日文「桜の下で、また会う日まで」、synopsis 仍是英文——玩家在对白之外读到的每一段文字（标题、简介、art_direction、场景标题）都来自 Director，而 Director 的 prompt 完全没有语言指令。已修（`1ed95d3`，同时把 id 字段钉死为 ASCII，因为它们会变成 Ren'Py 标识符），端到端确认待下次付费运行。

**对 50 场档的重新外推**：按 $0.1375/场，50 场约 **$6.9**（此前外推 $17.9）；墙钟按 128 秒/场、并发 3 约 **1.8 小时**。运行记录：`demo_output/real_20260919/tier_12_rerun/`。
