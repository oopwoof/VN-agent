...
| CI/CD | GitHub Actions (ruff + mypy + 972 pytest + coverage ≥60%) + Docker |
...
tests/                   # 972 pytest cases across 20+ test modules
```
...
```bash
uv run pytest -m "not slow"                         # 971 pass, 1 skipped (real-API test), 0 failures
